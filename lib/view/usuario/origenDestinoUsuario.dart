...
import 'origenUsuario.dart';

class OrigenDestinoUsuario extends StatefulWidget{
  final origen;
  final destino;
  final puntuacion;
  final tipoTransporte;

  OrigenDestinoUsuario({Key? key,this.origen, this.destino,this.puntuacion,this.tipoTransporte}) : super(key: key);
  @override
  _MapViewState createState() => _MapViewState();
}

class _MapViewState extends State<OrigenDestinoUsuario>{
  ...
  void _getObras() async{
    DBconn().getPolygonObras().then((result){
      for(var row in result){
        ...
        widget.origen == ' ' ? _setPolygon(polygonLatLngs) : null;
      }
      widget.origen == ' ' ? //mapController se crea en build y este metodo puede que se ejecute en initState()
  ...
  void initState() {
    ...
    widget.origen == ' ' ? _recibeParametros = false : _recibeParametros = true;
    if(_recibeParametros) {
      _tipoTransporte=widget.tipoTransporte;
      dibujarRuta(widget.origen, widget.destino); //TODO pasarle tipo transporte
      _getObras();
    }
    else{
      _tipoTransporte='Coche';
    }
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    String destino=widget.destino;
    String? puntuacion=widget.puntuacion;
    ...
    if(_recibeParametros){
      startAddressController.text =origen;
      destinationAddressController.text=destino;
      _recibeParametros=false;
    }
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          SizedBox(height: 2),
                          Visibility(
                            visible: puntuacion == null ? false : true,
                            child: Text(
        ...
        bottomNavigationBar: NavigationBar(
          onDestinationSelected: (int index) {
            if(index == 0){
              Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => OrigenUsuario()
                  )
              );
              setState(() {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Align(
        ...
        child: Padding(
          padding: const EdgeInsets.only(top: 152.5, right: 95.0),
          child: ClipOval(
            child: Material(
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Align(
        ...
        child: Padding(
          padding: const EdgeInsets.only(top: 152.5, left: 45.0),
          child: ClipOval(
            child: Material(
              color: activado ? Colors.blue.shade900: Colors.blue.shade500, // button color
              child: InkWell(
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Align(
        alignment: Alignment.topLeft,
        child: Padding(
          padding: const EdgeInsets.only(top: 152.5, left: 95.0),
          child: ClipOval(
            child: Material(
              color: activado ?   Colors.blue.shade900: Colors.blue.shade500, // button color
              child: InkWell(
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Align(
        alignment: Alignment.topRight,
        child: Padding(
          padding: const EdgeInsets.only(top: 152.5, right: 45.0),
          child: ClipOval(
            child: Material(
              color: activado ?  Colors.blue.shade900: Colors.blue.shade500, // button color
              child: InkWell(
