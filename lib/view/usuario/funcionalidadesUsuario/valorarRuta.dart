...
class ValorarRuta extends StatefulWidget {
  @override
  _MapViewState createState() => _MapViewState();
}

class _MapViewState extends State<ValorarRuta> {
  CameraPosition _camaraBilbao = CameraPosition(
      target: LatLng(43.263101512963964, -2.9351218790702007), zoom: 12.8);
  ...
  void addIconAscensor() {
    BitmapDescriptor.fromAssetImage(
        const ImageConfiguration(), "assets/images/ascensor.png")
        .then(
            (icon) {
          setState(() {
            markerAscensor = icon;
  ...
  void addIconAscensorPago() {
    BitmapDescriptor.fromAssetImage(
        const ImageConfiguration(), "assets/images/ascensorPago.png")
        .then(
            (icon) {
          setState(() {
  ...
  void addIconEscaleraMec() {
    BitmapDescriptor.fromAssetImage(const ImageConfiguration(),
        "assets/images/Marcador_escaleraMec_txiki.png")
        .then(
            (icon) {
          setState(() {
  ...
  void addIconStart() {
    BitmapDescriptor.fromAssetImage(
        const ImageConfiguration(), "assets/images/markerA.png")
        .then(
            (icon) {
          setState(() {
            markerStart = icon;
  ...
  void addIconDestination() {
    BitmapDescriptor.fromAssetImage(
        const ImageConfiguration(), "assets/images/markerB.png")
        .then(
            (icon) {
          setState(() {
  ...
  void initState() {
    ...
    addIconDestination();
    botonesCapaDatos =
        CapasDatos(_tratarPulsacionBtnCapas, _tratarPulsacionBtnMyLocation);
    super.initState();
  }
                        ...
                        children: <Widget>[
                          ...
                          ElevatedButton(
                            ...
                            child: Padding(
                              ...
                              child: Icon(Icons
                                  .star), //se puede quitar esto y poner el texto
                              /*Text(
  ...
  TravelMode _modoBusqueda() {
    switch (_tipoTransporte) {
      ...
      default:
        return TravelMode
            .driving; //default añadido para que no pueda devolver en ningún caso null
    }
  }
  ...
  TextEditingController puntuacionCrtl = new TextEditingController();

  TextEditingController valoracionCrtl = new TextEditingController();


  Future<void> _showAlertDialog1(String startAddress, double startLatitude,
      double startLongitude, String destinationAddress,
      double destinationLatitude, double destinationLongitude,
      String tipoTransporte) async {
            ...
            return AlertDialog(
              content: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      TextFormField(
                        controller: puntuacionCrtl,
                        decoration: new InputDecoration(
                          labelText: 'Escribe un numero entre 1 y 10',
                        ),
                      ),
                      TextFormField(
                        controller: valoracionCrtl,
                        decoration: new InputDecoration(
              ...
              actions: <Widget>[
                ElevatedButton(
                  child: Text(
                    "Valorar", style: TextStyle(color: Colors.blue),),
                  onPressed: () async {
                    // Do something like updating SharedPreferences or User Settings etc.
                    if (puntuacionCrtl.text.isEmpty) {
                      _showAlertDialog3();
                    }
                    else {
                      bool bien = await validate(
                          puntuacionCrtl.text,
                          valoracionCrtl.text,
                          startAddress,
                          startLatitude,
  ...
  Future<bool> validate(String value, String descripcion, String startAddress,
      double startLatitude, double startLongitude, String destinationAddress,
      double destinationLatitude, double destinationLongitude,
      String tipoTransporte) async {
    bool bien = false;
    String pattern = r'(^[a-zA-Z. \s À-ÿ , . -]*$)';
    ...
    if ((value == '0' || value == '1' || value == '2' || value == '3' ||
        ...
        value == '9' || value == '10')
        && regExp.hasMatch(descripcion)) {
      Navigator.of(context).pop();

      bool insertado = await DBconn().valorarRuta(
          startAddress,
          startLatitude,
          startLongitude,
          destinationAddress,
  ...
  void _showAlertDialog2() {
    showDialog(
        ...
        builder: (buildcontext) {
          return AlertDialog(
            title: Text("Valoración correcta"),
            content: Text("La ruta se ha valorado correctamente."),
            actions: <Widget>[
              ElevatedButton(
  ...
  void _showAlertDialog3() {
    showDialog(
        ...
        builder: (buildcontext) {
          return AlertDialog(
            title: Text("Valoración incorrecta"),
            content: Text(
                "La ruta no se ha valorado correctamente, intentalo de nuevo."),
            actions: <Widget>[
              ElevatedButton(
                child: Text("Cerrar", style: TextStyle(color: Colors.blue),),
                onPressed: () {
  ...
  void _showAlertDialog4() {
    showDialog(
        ...
        builder: (buildcontext) {
          return AlertDialog(
            title: Text("Valoración incorrecta"),
            content: Text("Esta ruta ya la habias valorado previamente."),
            actions: <Widget>[
              ElevatedButton(
                child: Text("Cerrar", style: TextStyle(color: Colors.blue),),
                onPressed: () {
